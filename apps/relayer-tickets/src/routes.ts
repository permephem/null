...
export async function registerRoutes(app: FastifyInstance) {
  ...
  app.post("/tickets/transfer", async (req, reply) => {
    ...
    if (!current) return reply.code(404).send({ error: "UNKNOWN_TICKET" });
    if (current.state === "REVOKED") return reply.code(400).send({ error: "REVOKED" });

    const fromTag = holderTagHmac(body.fromIdentifier, CONFIG.VENUE_HMAC_KEY);
    if (fromTag !== current.holderTag) return reply.code(403).send({ error: "NOT_CURRENT_HOLDER" });
    ...
    const canonTx = await canon.anchorTickets({
      ticket_id_commit: body.ticketIdCommit as any,
      policy_commit: current.policyCommit as any,
      holder_tag: toTag as any,
      assurance: body.assurance ?? 1,
      uri,
  ...
  app.post("/tickets/revoke", async (req, reply) => {
    ...
    const current = getState(body.ticketIdCommit);
    if (!current) return reply.code(404).send({ error: "UNKNOWN_TICKET" });
    if (current.state === "REVOKED") return reply.code(200).send({ canonTx: current.lastCanonTx });
    ...
    const canonTx = await canon.anchorTickets({
      ticket_id_commit: body.ticketIdCommit as any,
      policy_commit: current.policyCommit as any,
      holder_tag: current.holderTag as any, // holder tag at time of revoke (optional)
      assurance: body.assurance ?? 1,
      uri,

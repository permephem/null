...
User submits a Null Warrant JSON document to the relayer.

Relayer validates schema, canonicalizes JSON, computes digest and subject tag.

Relayer anchors warrant hash on-chain (Canon Registry) with fee payment.

Relayer returns a response containing the digest, subject tag, and transaction details.

4.2 Attestation
Implementer records deletion internally and issues a Deletion Attestation.

Attestation undergoes the same canonicalization and anchoring process.

Anchoring emits an event, providing auditability of the implementer’s action.

4.3 Receipt (Mask SBT) – Optional
After successful anchoring, the relayer (if configured) mints a Mask SBT to the subject.

SBT records the receipt hash, timestamp, and optional metadata.

Because SBTs are soulbound, they serve as a non-transferable proof of deletion.

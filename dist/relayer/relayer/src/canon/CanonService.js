...
export class CanonService {
    ...
    async anchorWarrant(warrantHash, subjectHandleHash, enterpriseHash, enterpriseId, warrantId, _controllerDidHash, _subjectTag, _assurance) {
        try {
            ...
            const tx = await this.contract.anchorWarrant(warrantHash, subjectHandleHash, enterpriseHash, enterpriseId, warrantId, { value: ethers.parseEther('0.01') } // Pay the base fee
            );
    ...
    async anchorAttestation(attestationHash, warrantHash, enterpriseHash, enterpriseId, attestationId, _controllerDidHash, _subjectTag, _assurance) {
        try {
            logger.info('Anchoring attestation to canon registry', { attestationId });
            const tx = await this.contract.anchorAttestation(attestationHash, warrantHash, enterpriseHash, enterpriseId, attestationId, { value: ethers.parseEther('0.01') } // Pay the base fee
            );
            const receipt = await tx.wait();
            if (!receipt) {
                throw new Error('Transaction receipt is null');
...
//# sourceMappingURL=CanonService.js.map
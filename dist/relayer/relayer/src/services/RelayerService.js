...
export class RelayerService {
    canonService;
    sbtService;
    constructor(canonService, sbtService, _emailService) {
    ...
    async processWarrant(warrant) {
        ...
        try {
            ...
            const subjectTag = CryptoService.generateSubjectTag(warrant.subject.subject_handle, warrant.enterprise_id, warrant.warrant_id);
            ...
            while (retryCount < maxRetries) {
                try {
                    anchorTxHash = await this.canonService.anchorWarrant(warrantDigest, warrantDigest, // subjectHandleHash placeholder
                    warrantDigest, // enterpriseHash placeholder
                    warrant.enterprise_id, warrant.warrant_id, warrantDigest, // controllerDidHash placeholder
                    subjectTag, this.determineAssuranceLevel(warrant));
                    break;
                }
                catch (error) {
                    retryCount++;
    ...
    async processAttestation(attestation) {
        try {
            ...
            const anchorResult = await this.canonService.anchorAttestation(attestationDigest, attestationDigest, // warrantHash placeholder
            attestationDigest, // enterpriseHash placeholder
            attestation.enterprise_id, attestation.attestation_id, attestationDigest, // controllerDidHash placeholder
            attestationDigest, // subjectTag placeholder
            this.determineAssuranceLevel({ evidence_requested: [] }));
    ...
    async validateWarrant(warrant) {
        try {
            ...
            if (warrant.signature?.sig && warrant.signature?.kid) {
                ...
                const signatureValid = await CryptoService.verifySignature(canonicalData, warrant.signature.sig, warrant.signature.kid, warrant.signature.alg || 'EdDSA');
                if (!signatureValid) {
                    return {
                        valid: false,
    ...
    async validateAttestation(attestation) {
        try {
            ...
            if (attestation.signature?.sig && attestation.signature?.kid) {
                ...
                const signatureValid = await CryptoService.verifySignature(canonicalData, attestation.signature.sig, attestation.signature.kid, attestation.signature.alg || 'EdDSA');
                if (!signatureValid) {
                    return {
                        valid: false,
                        error: 'Invalid signature',
     ...
     * @returns The digest
     */
    computeAttestationDigest(attestation) {
     ...
     */
    determineAssuranceLevel(warrant) {
     ...
     */
    async sendWarrantToEnterprise(warrant) {
     ...
     */
    async generateReceipt(attestation) {
        try {
            const receipt = {
                type: 'MaskReceipt@v0.2',
                receipt_id: `receipt_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
                warrant_hash: this.computeWarrantDigest({ warrant_id: attestation.warrant_id }),
...
//# sourceMappingURL=RelayerService.js.map